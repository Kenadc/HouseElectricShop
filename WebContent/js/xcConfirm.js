...
(function($){
	...
	window.wxc.xcConfirm1 = function(popHtml, type, options) {
	    var btnType = window.wxc.xcConfirm.btnEnum;
		var eventType = window.wxc.xcConfirm.eventEnum;
		var popType = {
			info: {
		...
		function doOk(){
			...
			$.ajax({
			url : "/HouseElectricShop/userupdatebytel.action",
			data:{
				"tel":$("#telForgetPwd").val(),
				"password":$("#newTelForgetPwd").val()
			},
			type : "post",
			async:true,
			success : function(data) {
				$('.alert').html('密码修改成功').show().delay(2500).fadeOut(); 
