...
<!--End Menu End--> 
<div class="i_bg" id="BuyCar_Twos">  
    <!-- <div class="content mar_20">
    	<img src="images/img2.jpg" />        
    </div> -->
    
    <!--Begin 第二步：确认订单信息 Begin -->
    <form name=alipayment action="/HouseElectricShop/setAttributes.action" method=post
			target="_blank">
	    <div class="content mar_20">
	    	<div class="two_bg">
	        	<div class="two_t">
	            ...
	            <table border="0" class="car_tab" style="width:1110px;" cellspacing="0" cellpadding="0">
	              <tr>
	                ...
	                <td class="car_th" width="130">小计</td>
					 ...
					 <tr v-for="p in proInfos">
		                <td><div class="c_s_img"><img v-bind:src="[p.imgUrl]" width="73" height="73" /></div>
		                   		{{p.name}}
		                </td>
		                <td align="center">{{p.brand}}</td>
		                <td align="center">{{p.num}}</td>
		                <td align="center" style="color:#ff4e00;">￥{{p.price * p.num}}</td>
		              </tr>
	            ...
	            <table border="0" class="peo_tab" style="width:1110px;" cellspacing="0" cellpadding="0">
	              <tr>
	                ...
	                <td width="395">{{productNames}}</td>
	                <td class="p_td" width="160">电子邮件</td>
	                <td width="395"><input type="text" name="email" value="${loginInfo.email }" style="font-size: 14px;"/></td>
	              </tr>
	              <tr>
	                <td class="p_td">地址信息</td>
	                <td><input type="text" name="addressInfo" value="${loginInfo.liveAddress }" style="font-size: 14px;"/></td>
	                <td class="p_td">邮政编码</td>
	                <td><input type="text" name="post" style="font-size: 14px;"/></td>
	              </tr>
	              <tr>
	                <td class="p_td">电话</td>
	                <td><input type="text" name="phone" value="${loginInfo.tel }" style="font-size: 14px;"/></td>
	                <td class="p_td">手机</td>
	                <td><input type="text" name="tel" value="${loginInfo.tel }" style="font-size: 14px;"/></td>
	              </tr>
	              <tr>
	            ...
	            <table border="0" style="width:900px; margin-top:20px;" cellspacing="0" cellpadding="0">
	              <tr>
	                <td align="right">
	                    ...
	                    商品总价: <font color="#ff4e00">￥{{allPrice}}</font>  + 配送费用: <font color="#ff4e00">￥0.00</font>
	                </td>
	              </tr>
	              <tr height="70">
	                <td align="right">
	                	<b style="font-size:14px;">应付款金额：<span style="font-size:22px; color:#ff4e00;">￥{{allPrice}}</span></b>
	                	<input id="WIDtotal_amount" name="WIDtotal_amount" :value="allPrice"  type="hidden"/>
	                	<input id="WIDsubject" name="WIDsubject"  value="${loginInfo.liveAddress }" type="hidden"/>
	                	<input id="WIDout_trade_no" name="WIDout_trade_no" :value="nowDate" type="hidden"/>
	                	<input id="product_id" name="id" value="10" type="hidden"/>
	                	<input name="carInfos" :value="productIdAndProNums" type="hidden"/>
	                </td>
	              </tr>
	              <tr height="70">
	                <td align="right">
	                	...
	                	px;" style="text-align: center;" onclick="sureOrders()">确认结算</button>
	                </td>
	              </tr>
	            </table>
...
var app = new Vue({
	...
	mounted : function() {
		var productIdAndProNum = localStorage.getItem("productIdAndProNum");
		this.productIdAndProNums = productIdAndProNum;
		console.log(productIdAndProNum);
		
		$.ajax({    
	        type: "post",   
	        url: "/HouseElectricShop/productFindByIdGoToOrderInfos.action",   
	        data: productIdAndProNum ,    
	        ...
	        success: function (result) {    
				...
				for(var j = 0;j<result.length;j++){
					if(j != result.length){
						productName = productName + result[j].name + "、"; 
					}else{
						productName = productName + result[j].name;
					}
					totalPrice = totalPrice + parseInt(result[j].price) * parseInt(result[j].num);
					var arr = result[j].productPhotosList;
					for(var i = 0;i<arr.length;i++){
						if(arr[i].imgType == "1"){
							result[j].imgUrl = IMGURL+arr[i].imageUrl;
						}
					}
				}
				app.allPrice = totalPrice;
	        	app.proInfos = result;
	        	console.log(JSON.stringify(app.proInfos));
	        	app.productNames = productName;
	        } 
	    })  
